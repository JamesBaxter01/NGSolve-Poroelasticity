...
def ConfinedCompression(G, nu, viscosity, alpha, phi, k, chi, rho, g, dt, dt_max, dt_growth, R, H, u_max , compression_time, t_end, nx, ny, grading, order, tol, Confined=True, DrawResults=False):
    ...
    while t < t_end_star:
        ...
        F[0].data = b_f.vec
        F[1].data = ((alpha/dt_star) * a_Q.mat.T * u_old_star.vec + 
                    (S_star/dt_star) * a_S.mat * p_old_star.vec + 
                    b_q.vec)
        ...
        a_Schur += ((S_star + alpha**2) / dt_star) * p * q * twopi * r * dx  # augmented M
        a_Schur.Assemble()
        ...
        GMRes(A=A, b=rhs_resid, pre=pre_C, x=correction, tol=tol, printrates=False, maxsteps=500)
        sol.data += correction
